...
export async function GET(req:NextRequest){
  ...
  if(!url||!key)return NextResponse.json({product:null,error:"Supabase env eksik"},{status:503});
  ...
  if(error)return NextResponse.json({product:null,error:error.message},{status:500});
